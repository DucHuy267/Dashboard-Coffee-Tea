...
export default function DashboardHomePage() {
  ...
  const stats = useMemo(() => {
    ...
    return { today: todayCount, month: monthCount, revenueToday, revenueMonth, profitToday, profitMonth };
  }, [orders]);
  ...
  // ================================
  const columns = [
    { title: "STT", key: "stt", render: (_: unknown, record: IOrder, index: number) => index + 1 },
  ...
  const cardStyle = { borderRadius: 12, boxShadow: "0 6px 15px rgba(0,0,0,0.1)", padding: 20, fontWeight: 500, fontSize: 16 };
  ...
  return (
    <AdminLayout>
      ...
      <Row gutter={[16, 16]} className="mb-4">
        <Col xs={24} sm={12} md={8}>
          <Card title="Đơn hàng hôm nay" style={cardStyle}>
            {loadingOrders ? <Spin /> : stats.today} đơn
          </Card>
        </Col>
        <Col xs={24} sm={12} md={8}>
          <Card title="Đơn hàng tháng này" style={cardStyle}>
            {loadingOrders ? <Spin /> : stats.month} đơn
          </Card>
        </Col>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={12}>
          <Card title="Doanh thu theo ngày (tháng này)" style={cardStyle}>
        ...
        <Col xs={24} lg={12}>
          <Card title="Số lượng đơn theo tháng" style={cardStyle}>
            <ResponsiveContainer width="100%" height={300}>
